  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(err.message || 'Error updating tour');
      }
    } catch (error) {
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        <header className="mb-10 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-serif font-bold text-white mb-2">Edit Tour</h1>
              <p className="text-white/40">Modify tour details.</p>
            </div>
          </div>
        </header>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-3 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <input required type="text" value={formData.duration} onChange={e => setFormData({...formData, duration: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-white focus:border-primary/50" />
                </div>
                <div className="space-y-2">
